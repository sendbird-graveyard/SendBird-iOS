...
- (void)loadMessages:(long long)ts initial:(BOOL)initial {
    ...
    [[SendBird queryMessageListInChannel:[self.channel url]] prevWithMessageTs:ts andLimit:50 resultBlock:^(NSMutableArray *queryResult) {
        ...
        if (initial) {
            [self scrollToBottomAnimated:NO];
            [SendBird joinChannel:[self.channel url]];
