...
class ChattingView: ReusableViewFromXib, UITableViewDelegate, UITableViewDataSource, UITextViewDelegate {
    ...
    var resendableFileData: [String:[String:Any]] = [:]
    var preSendFileData: [String:[String:Any]] = [:]
    ...
    func setup() {
        self.messagesTableView?.contentInset = UIEdgeInsets.init(top: 0, left: 0, bottom: 10, right: 0)
    ...
    func configureChattingView(channel: SBDBaseChannel?) {
        ...
        self.messagesTableView?.register(IncomingUserMessageTableViewCell.nib(), forCellReuseIdentifier: IncomingUserMessageTableViewCell.cellReuseIdentifier())
        self.messagesTableView?.register(OutgoingUserMessageTableViewCell.nib(), forCellReuseIdentifier: OutgoingUserMessageTableViewCell.cellReuseIdentifier())
        self.messagesTableView?.register(NeutralMessageTableViewCell.nib(), forCellReuseIdentifier: NeutralMessageTableViewCell.cellReuseIdentifier())
        self.messagesTableView?.register(IncomingFileMessageTableViewCell.nib(), forCellReuseIdentifier: IncomingFileMessageTableViewCell.cellReuseIdentifier())
        self.messagesTableView?.register(OutgoingImageFileMessageTableViewCell.nib(), forCellReuseIdentifier: OutgoingImageFileMessageTableViewCell.cellReuseIdentifier())
        self.messagesTableView?.register(OutgoingFileMessageTableViewCell.nib(), forCellReuseIdentifier: OutgoingFileMessageTableViewCell.cellReuseIdentifier())
        self.messagesTableView?.register(IncomingImageFileMessageTableViewCell.nib(), forCellReuseIdentifier: IncomingImageFileMessageTableViewCell.cellReuseIdentifier())
        self.messagesTableView?.register(IncomingVideoFileMessageTableViewCell.nib(), forCellReuseIdentifier: IncomingVideoFileMessageTableViewCell.cellReuseIdentifier())
        self.messagesTableView?.register(OutgoingVideoFileMessageTableViewCell.nib(), forCellReuseIdentifier: OutgoingVideoFileMessageTableViewCell.cellReuseIdentifier())
        
        self.messagesTableView?.register(IncomingGeneralUrlPreviewMessageTableViewCell.nib(), forCellReuseIdentifier: IncomingGeneralUrlPreviewMessageTableViewCell.cellReuseIdentifier())
        self.messagesTableView?.register(OutgoingGeneralUrlPreviewMessageTableViewCell.nib(), forCellReuseIdentifier: OutgoingGeneralUrlPreviewMessageTableViewCell.cellReuseIdentifier())
        self.messagesTableView?.register(OutgoingGeneralUrlPreviewTempMessageTableViewCell.nib(), forCellReuseIdentifier: OutgoingGeneralUrlPreviewTempMessageTableViewCell.cellReuseIdentifier())
    ...
    func scrollToBottom(force: Bool) {
        ...
        self.messagesTableView?.scrollToRow(at: IndexPath.init(row: self.messages.count - 1, section: 0), at: UITableView.ScrollPosition.bottom, animated: false)
    }
    ...
    func scrollToPosition(position: Int) {
        ...
        self.messagesTableView?.scrollToRow(at: IndexPath.init(row: position, section: 0), at: UITableView.ScrollPosition.top, animated: false)
    }
        ...
        if msg is SBDUserMessage {
            ...
            if sender?.userId == SBDMain.getCurrentUser()?.userId {
                ...
                if userMessage.customType == "url_preview" {
                    ...
                    if let imageUrl = (cell as! OutgoingGeneralUrlPreviewMessageTableViewCell).previewData["image"] as? String {
                        ...
                        if let requestId = userMessage.requestId(), self.preSendMessages[requestId] != nil {
                            (cell as! OutgoingGeneralUrlPreviewMessageTableViewCell).showSendingStatus()
                        }
                        else if let requestId = userMessage.requestId(), self.resendableMessages[requestId] != nil {
            ...
            if sender?.userId == SBDMain.getCurrentUser()?.userId {
                ...
                if fileMessage.type.hasPrefix("video") {
                    ...
                    if let requestId = fileMessage.requestId(), self.preSendMessages[requestId] != nil {
                        (cell as! OutgoingVideoFileMessageTableViewCell).showSendingStatus()
                    }
                    else if let requestId = fileMessage.requestId(), self.resendableMessages[requestId] != nil {
                    ...
                    if let requestId = fileMessage.requestId(), self.preSendMessages[requestId] != nil {
                        ...
                        (cell as! OutgoingImageFileMessageTableViewCell).setImageData(data: self.preSendFileData[requestId]?["data"] as! Data, type: self.preSendFileData[requestId]?["type"] as! String)
                    } else if let requestId = fileMessage.requestId(), self.resendableMessages[requestId] != nil {
                        (cell as! OutgoingImageFileMessageTableViewCell).showMessageControlButton()
                        (cell as! OutgoingImageFileMessageTableViewCell).setImageData(data: self.resendableFileData[requestId]?["data"] as! Data, type: self.resendableFileData[requestId]?["type"] as! String)
                        (cell as! OutgoingImageFileMessageTableViewCell).hasImageCacheData = true
                    } else {
                        if let requestId = fileMessage.requestId(), !fileMessage.url.isEmpty && self.preSendFileData[requestId] != nil {
                            (cell as! OutgoingImageFileMessageTableViewCell).setImageData(data: self.preSendFileData[requestId]?["data"] as! Data, type: self.preSendFileData[requestId]?["type"] as! String)
                            (cell as! OutgoingImageFileMessageTableViewCell).hasImageCacheData = true
