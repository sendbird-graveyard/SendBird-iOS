...
@implementation OpenChannelViewController
...
- (void)loadMessages:(long long)ts initial:(BOOL)initial {
    ...
    __weak OpenChannelViewController *weakSelf = self;
    [self.previousMessageQuery loadPreviousMessagesWithLimit:30 reverse:!initial completionHandler:^(NSArray<SBDBaseMessage *> * _Nullable messages, SBDError * _Nullable error) {
        OpenChannelViewController *strongSelf = weakSelf;
        if (error != nil) {
        ...
        if (messages != nil && [messages count] > 0) {
            ...
            for (SBDBaseMessage *message in messages) {
                ...
                if ([message isKindOfClass:[SBDUserMessage class]]) {
                    ...
                    NSDate *msgDate = [NSDate dateWithTimeIntervalSince1970:((double)((SBDUserMessage *)message).createdAt) / 1000.0];
                    ...
                    NSDate *msgDate = [NSDate dateWithTimeIntervalSince1970:((double)((SBDFileMessage *)message).createdAt) / 1000.0];
...
{
    JSQSBMessage *currentMessage = self.messages[indexPath.item];
                      ...
                      date:(NSDate *)date
{
    if ([text length] > 0) {
        [self.channel sendUserMessage:text completionHandler:^(SBDUserMessage * _Nullable userMessage, SBDError * _Nullable error) {
            if (error != nil) {
                UIAlertController *vc = [UIAlertController alertControllerWithTitle:@"Error" message:[NSString stringWithFormat:@"%ld: %@", error.code, error.domain] preferredStyle:UIAlertControllerStyleAlert];
                UIAlertAction *closeAction = [UIAlertAction actionWithTitle:@"Close" style:UIAlertActionStyleCancel handler:nil];
                ...
                NSDate *msgDate = [NSDate dateWithTimeIntervalSince1970:(double)[userMessage createdAt] / 1000.0];
        ...
        if (CFStringCompare ((CFStringRef) mediaType, kUTTypeImage, 0) == kCFCompareEqualTo) {
            ...
            [strongSelf.channel sendFileMessageWithBinaryData:imageFileData filename:imageName type:imageType size:imageFileData.length data:@"" completionHandler:^(SBDFileMessage * _Nullable fileMessage, SBDError * _Nullable error) {
                ...
                if (fileMessage != nil) {
                    ...
                    NSDate *msgDate = [NSDate dateWithTimeIntervalSince1970:(double)[fileMessage createdAt] / 1000.0];
...
- (void)didFailReconnection {
    ...
    UIAlertController *vc = [UIAlertController alertControllerWithTitle:@"Error" message:@"Reconnection failed" preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *closeAction = [UIAlertAction actionWithTitle:@"Close" style:UIAlertActionStyleCancel handler:nil];
    [vc addAction:closeAction];
    [self presentViewController:vc animated:YES completion:nil];
}
...
- (void)channel:(SBDBaseChannel * _Nonnull)sender didReceiveMessage:(SBDBaseMessage * _Nonnull)message {
    ...
    if ([message isKindOfClass:[SBDUserMessage class]]) {
        ...
        NSString *senderImage = [[((SBDUserMessage *)message) sender] profileUrl];
        NSString *senderName = [[((SBDUserMessage *)message) sender] nickname];
        NSDate *msgDate = [NSDate dateWithTimeIntervalSince1970:(double)[message createdAt] / 1000.0];
        NSString *messageText = [((SBDUserMessage *)message) message];
