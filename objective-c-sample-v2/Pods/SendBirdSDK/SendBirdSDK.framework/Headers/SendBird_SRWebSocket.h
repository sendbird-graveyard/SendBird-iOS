...
typedef NS_ENUM(NSInteger, SendBird_SRReadyState) {
...
typedef NS_ENUM(NSInteger, SendBird_SRStatusCode) {
    // 0-999: Reserved and not used.
    SendBird_SRStatusCodeNormal = 1000,
    SendBird_SRStatusCodeGoingAway = 1001,
    SendBird_SRStatusCodeProtocolError = 1002,
    SendBird_SRStatusCodeUnhandledType = 1003,
    // 1004 reserved.
    SendBird_SRStatusNoStatusReceived = 1005,
    SendBird_SRStatusCodeAbnormal = 1006,
    SendBird_SRStatusCodeInvalidUTF8 = 1007,
    SendBird_SRStatusCodePolicyViolated = 1008,
    SendBird_SRStatusCodeMessageTooBig = 1009,
    SendBird_SRStatusCodeMissingExtension = 1010,
    SendBird_SRStatusCodeInternalError = 1011,
    SendBird_SRStatusCodeServiceRestart = 1012,
    SendBird_SRStatusCodeTryAgainLater = 1013,
    // 1014: Reserved for future use by the WebSocket standard.
    SendBird_SRStatusCodeTLSHandshake = 1015,
...
@class SendBird_SRWebSocket;
@class SendBird_SRSecurityPolicy;

/**
 Error domain used for errors reported by SRWebSocket.
 */
extern NSString *const SendBird_SRWebSocketErrorDomain;

/**
 Key used for HTTP status code if bad response was received from the server.
 */
extern NSString *const SendBird_SRHTTPResponseErrorKey;

@protocol SendBird_SRWebSocketDelegate;

///--------------------------------------
#pragma mark - SendBird_SRWebSocket
///--------------------------------------
...
/**
 Current ready state of the socket. Default: `SendBird_SR_CONNECTING`.

 This property is Key-Value Observable and fully thread-safe.
 */
@property (atomic, assign, readonly) SendBird_SRReadyState readyState;
 ...
 */
- (instancetype)initWithURLRequest:(NSURLRequest *)request securityPolicy:(SendBird_SRSecurityPolicy *)securityPolicy;
...
/**
 ...
 @param securityPolicy Policy object describing transport security behavior.
 */
- (instancetype)initWithURLRequest:(NSURLRequest *)request protocols:(nullable NSArray<NSString *> *)protocols securityPolicy:(SendBird_SRSecurityPolicy *)securityPolicy NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 @param securityPolicy Policy object describing transport security behavior.
 */
- (instancetype)initWithURL:(NSURL *)url securityPolicy:(SendBird_SRSecurityPolicy *)securityPolicy;
...
///--------------------------------------
#pragma mark - SendBird_SRWebSocketDelegate
///--------------------------------------
...
/**
 ...
 @param webSocket An instance of `SendBird_SRWebSocket` that received a message.
 @param message   Received message. Either a `String` or `NSData`.
 */
- (void)webSocket:(SendBird_SRWebSocket *)webSocket didReceiveMessage:(id)message;
 ...
 */
- (void)webSocket:(SendBird_SRWebSocket *)webSocket didReceiveMessageWithString:(NSString *)string;
 ...
 */
- (void)webSocket:(SendBird_SRWebSocket *)webSocket didReceiveMessageWithData:(NSData *)data;
...
/**
 ...
 @param webSocket An instance of `SendBird_SRWebSocket` that was open.
 */
- (void)webSocketDidOpen:(SendBird_SRWebSocket *)webSocket;

/**
 Called when a given web socket encountered an error.

 @param webSocket An instance of `SendBird_SRWebSocket` that failed with an error.
 @param error     An instance of `NSError`.
 */
- (void)webSocket:(SendBird_SRWebSocket *)webSocket didFailWithError:(NSError *)error;

/**
 Called when a given web socket was closed.

 @param webSocket An instance of `SendBird_SRWebSocket` that was closed.
 ...
 */
- (void)webSocket:(SendBird_SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(nullable NSString *)reason wasClean:(BOOL)wasClean;

/**
 Called on receive of a ping message from the server.

 @param webSocket An instance of `SendBird_SRWebSocket` that received a ping frame.
 @param data      Payload that was received or `nil` if there was no payload.
 */
- (void)webSocket:(SendBird_SRWebSocket *)webSocket didReceivePingWithData:(nullable NSData *)data;

/**
 Called when a pong data was received in response to ping.

 @param webSocket An instance of `SendBird_SRWebSocket` that received a pong frame.
 @param pongData  Payload that was received or `nil` if there was no payload.
 */
- (void)webSocket:(SendBird_SRWebSocket *)webSocket didReceivePong:(nullable NSData *)pongData;
...
/**
 ...
 @param webSocket An instance of `SendBird_SRWebSocket` that received a text frame.

 @return `YES` if text frame should be converted to UTF-8 String, otherwise - `NO`. Default: `YES`.
 */
- (BOOL)webSocketShouldConvertTextFrameToString:(SendBird_SRWebSocket *)webSocket NS_SWIFT_NAME(webSocketShouldConvertTextFrameToString(_:));
